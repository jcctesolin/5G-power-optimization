...
L = 4;      % número de APs
sigma2 = 1e-9; % ruído
P_max = 10 * ones(L,1);% potência máxima por AP (W)
P_min = 1 * ones(L,1);% potência minima por AP (W)
...
while eta_diff > tol && iter < max_iter
    ...
    [p_opt, fval] = fmincon(obj, p0, [], [], [], [], P_min, P_max, [], options);
